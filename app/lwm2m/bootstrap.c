...
prv_handleBootstrapReply(lwm2m_transaction_t * transaction, void * message)
{
...
bootstrap_initiating_request(lwm2m_context_t * context)
{
    char query[PRV_QUERY_BUFFER_LENGTH];
    int query_length = 0;
    lwm2m_transaction_t * transaction = NULL;

    query_length = c_snprintf(query, sizeof(query), "?ep=%s", context->endpointName);
    if (query_length <= 1)
    {
...
void ICACHE_FLASH_ATTR handle_bootstrap_response(lwm2m_context_t * context,
...
void ICACHE_FLASH_ATTR bootstrap_failed(lwm2m_context_t * context)
{
    reset_bootstrap_timer(context);
...
void ICACHE_FLASH_ATTR reset_bootstrap_timer(lwm2m_context_t * context)
{
    context->bsStart = lwm2m_gettime();
}

void ICACHE_FLASH_ATTR update_bootstrap_state(lwm2m_context_t * context,
...
coap_status_t ICACHE_FLASH_ATTR handle_bootstrap_finish(lwm2m_context_t * context,
                                      void * fromSessionH)
{
    if (context->bsState == BOOTSTRAP_PENDING)
    {
...
uint8_t ICACHE_FLASH_ATTR handle_bootstrap_request(lwm2m_context_t * contextP,
                                 lwm2m_uri_t * uriP,
                                 void * fromSessionH,
                                 coap_packet_t * message,
                                 coap_packet_t * response)
{
    ...
    c_memcpy(name, message->uri_query->data + QUERY_LENGTH, message->uri_query->len - QUERY_LENGTH);
...
void ICACHE_FLASH_ATTR lwm2m_set_bootstrap_callback(lwm2m_context_t * contextP,
...
static ICACHE_FLASH_ATTR void bs_result_callback(lwm2m_transaction_t * transacP,
                               void * message)
{
...
int ICACHE_FLASH_ATTR lwm2m_bootstrap_delete(lwm2m_context_t * contextP,
                           void * sessionH,
                           lwm2m_uri_t * uriP)
{
    ...
    {
        dataP->isUri = true;
        c_memcpy(&dataP->uri, uriP, sizeof(lwm2m_uri_t));
    }
...
int ICACHE_FLASH_ATTR lwm2m_bootstrap_write(lwm2m_context_t * contextP,
                          void * sessionH,
                          lwm2m_uri_t * uriP,
                          uint8_t * buffer,
                          size_t length)
...
int ICACHE_FLASH_ATTR lwm2m_bootstrap_finish(lwm2m_context_t * contextP,
                           void * sessionH)
{
    lwm2m_transaction_t * transaction;
    bs_data_t * dataP;
