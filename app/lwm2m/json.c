    ...
    for (index = 0 ; index < count ; index++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                c_memcpy(targetP + 1, tlvP[resIndex].value, tlvP[resIndex].length * sizeof(lwm2m_data_t));
                              ...
                              size_t bufferLen)
{
    ...
    res = c_snprintf((char *)(char *)buffer, bufferLen, JSON_RES_ITEM_TEMPLATE, tlvP->id);
    if (res <= 0 || res >= bufferLen) return -1;

    switch (tlvP->dataType)
    {
    case LWM2M_TYPE_STRING:
        res = c_snprintf((char *)buffer, bufferLen, JSON_ITEM_STRING_BEGIN);
        if (res <= 0 || res >= bufferLen) return -1;
        head = res;
        if (tlvP->length >= bufferLen - head) return -1;
        c_memcpy(buffer + head, tlvP->value, tlvP->length);
        head += tlvP->length;
        res = c_snprintf((char *)buffer + head, bufferLen - head, JSON_ITEM_STRING_END);
    ...
    case LWM2M_TYPE_TIME:
    {
        ...
        res = c_snprintf((char *)buffer, bufferLen, JSON_ITEM_INTEGER_TEMPLATE, value);
        if (res <= 0 || res >= bufferLen) return -1;
    }
    break;
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        res = c_snprintf((char *)buffer, bufferLen, JSON_ITEM_FLOAT_TEMPLATE, value);
        if (res <= 0 || res >= bufferLen) return -1;
    }
    break;
    ...
    case LWM2M_TYPE_BOOLEAN:
    {
        ...
        res = c_snprintf((char *)buffer, bufferLen, value?JSON_ITEM_BOOL_TRUE:JSON_ITEM_BOOL_FALSE);
        if (res <= 0 || res >= bufferLen) return -1;
    }
    break;
                         ...
                         uint8_t ** bufferP)
{
    ...
    c_memcpy(bufferJSON, JSON_HEADER, JSON_HEADER_SIZE);
    ...
    for (index = 0 ; index < size && head < PRV_JSON_BUFFER_SIZE ; index++)
    {
        ...
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
        {
            ...
            {
                res = c_snprintf((char *)bufferJSON + head, PRV_JSON_BUFFER_SIZE - head, JSON_INST_ITEM_TEMPLATE, tlvP[index].id, subTlvP[i].id);
        ...
        case LWM2M_TYPE_RESOURCE:
            res = c_snprintf((char *)bufferJSON + head, PRV_JSON_BUFFER_SIZE - head, JSON_RES_ITEM_TEMPLATE, tlvP[index].id);
            if (res <= 0 || res >= PRV_JSON_BUFFER_SIZE - head) return 0;
            head += res;
    ...
    c_memcpy(bufferJSON + head - 1, JSON_FOOTER, JSON_FOOTER_SIZE);
    head = head - 1 + JSON_FOOTER_SIZE;

    *bufferP = (uint8_t *)lwm2m_malloc(head);
    if (*bufferP == NULL) return 0;
    c_memcpy(*bufferP, bufferJSON, head);
