...
prv_getHeaderLength(uint16_t id,
                               size_t dataLen)
{
    int length;
...
static int ICACHE_FLASH_ATTR
prv_create_header(uint8_t * header,
                          ...
                          size_t * lengthP)
{
    ...
    c_memset(data_buffer, 0, _PRV_64BIT_BUFFER_SIZE);
...
lwm2m_opaqueToTLV(lwm2m_tlv_type_t type,
                      uint8_t* dataP,
                      size_t data_len,
                      uint16_t id,
                      uint8_t * buffer,
                      size_t buffer_len)
{
    ...
    os_memmove(buffer, header, header_len);

    os_memmove(buffer + header_len, dataP, data_len);

    return header_len + data_len;
}

int ICACHE_FLASH_ATTR
lwm2m_boolToTLV(lwm2m_tlv_type_t type,
                    bool value,
                    uint16_t id,
                    uint8_t * buffer,
                    size_t buffer_len)
...
int ICACHE_FLASH_ATTR
lwm2m_intToTLV(lwm2m_tlv_type_t type,
                   int64_t data,
                   uint16_t id,
                   uint8_t * buffer,
                   size_t buffer_len)
...
int ICACHE_FLASH_ATTR
lwm2m_decodeTLV(uint8_t * buffer,
                    size_t buffer_len,
...
int ICACHE_FLASH_ATTR
lwm2m_opaqueToInt(uint8_t * buffer,
                      size_t buffer_len,
                      int64_t * dataP)
{
    size_t i;
...
int ICACHE_FLASH_ATTR
lwm2m_opaqueToFloat(uint8_t * buffer,
                        size_t buffer_len,
                        double * dataP)
{
    switch (buffer_len)
    ...
    case 4:
    {
        ...
        c_memcpy(&temp, buffer, buffer_len);
#else
    ...
    case 8:
#ifdef LWM2M_BIG_ENDIAN
        c_memcpy(dataP, buffer, buffer_len);
#else
#ifdef LWM2M_LITTLE_ENDIAN
    {
        size_t i;
...
lwm2m_data_t * ICACHE_FLASH_ATTR
lwm2m_data_new(int size)
{
    lwm2m_data_t * dataP;
    ...
    if (dataP != NULL)
    {
        c_memset(dataP, 0, size * sizeof(lwm2m_data_t));
    }
    ...
    *dataP = NULL;
    ...
    while (0 != (result = lwm2m_decodeTLV((uint8_t*)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        ...
        if (size >= 1)
        {
            ...
            {
                c_memcpy(newTlvP, *dataP, size * sizeof(lwm2m_data_t));
                lwm2m_free(*dataP);
            }
        }
        *dataP = newTlvP;
...
int ICACHE_FLASH_ATTR
lwm2m_data_parse(uint8_t * buffer,
                     size_t bufferLen,
        ...
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            {
                ...
                else
                {
                    ...
                    index += headerLen;
                    c_memcpy(*bufferP + index, tmpBuffer, tmpLength);
                    index += tmpLength;
        ...
        case LWM2M_TYPE_RESOURCE:
            {
                ...
                {
                    index += headerLen;
                    c_memcpy(*bufferP + index, dataP[i].value, dataP[i].length);
...
void ICACHE_FLASH_ATTR
lwm2m_data_encode_int(int64_t value,
                           lwm2m_data_t * dataP)
{
    dataP->length = 0;
...
int ICACHE_FLASH_ATTR
lwm2m_data_decode_int(lwm2m_data_t * dataP,
                          int64_t * valueP)
{
    int result;
...
void ICACHE_FLASH_ATTR
lwm2m_data_encode_float(double value,
                             lwm2m_data_t * dataP)
{
    dataP->length = 0;
    ...
    else
    {
        ...
        if (dataP->value != NULL)
        {
            if (length == 4)
            {
                ...
                temp = value;
#ifdef LWM2M_BIG_ENDIAN
                c_memcpy(dataP->value, &temp, length);
#else
#ifdef LWM2M_LITTLE_ENDIAN
                {
                    int i;
            ...
            {
#ifdef LWM2M_BIG_ENDIAN
                c_memcpy(dataP->value, &value, length);
#else
#ifdef LWM2M_LITTLE_ENDIAN
                size_t i;
...
int ICACHE_FLASH_ATTR
lwm2m_data_decode_float(lwm2m_data_t * dataP,
                            double * valueP)
{
    int result;
...
void ICACHE_FLASH_ATTR
lwm2m_data_encode_bool(bool value,
                            lwm2m_data_t * dataP)
{
    dataP->length = 0;
...
int ICACHE_FLASH_ATTR
lwm2m_data_decode_bool(lwm2m_data_t * dataP,
                           bool * valueP)
{
...
void ICACHE_FLASH_ATTR
lwm2m_data_include(lwm2m_data_t * subDataP,
                        size_t count,
                        lwm2m_data_t * dataP)
{
